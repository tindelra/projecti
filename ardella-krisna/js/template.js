...
function ajaxCall(data, callback) {
            ...
            const result = {
                ...
                rsvp_content: `
                    <style>
                        ...
                        .rsvp-success .btn-submit { margin-top: 10px; width: 100%; max-width: 250px; padding: 14px 20px; background-color: var(--button-background-secondary); color: var(--button-text-secondary) !important; border: none !important; border-radius: 50px !important; font-family: var(--body-text-family); font-size: 1.2rem; font-weight: 600; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.2); }
                        @media (max-width: 960px) {
                            .rsvp-success { gap: 7px; }
                            .rsvp-success h3 { font-size: 3rem; }
                            .rsvp-success p { font-size: 1rem; color: var(--text-tertiary)}
    ...
    <h3 style=";color: var(--text-tertiary); calc(var(--heading-size));">Terima Kasih!</h3>
    <p style="color: var(--text-tertiary); calc(var(--body-text-size))">Konfirmasi kehadiran Anda sudah kami terima.</p>

    <div class="rsvp-action" style="justify-content: center; margin-top: 20px;">
        <button type="button" class="btn-submit" onclick="resetRSVPForm()" style="margin: 0;">
            Ubah jawaban
        </button>
    </div>
</div>`
...
function copyAccountNumber(text, btn) {
    ...
    if (btn) {
        ...
        // Show checkmark
        btn.innerHTML = `
            <svg width="20" height="20" fill="#4BB543" viewBox="0 0 256 256">
